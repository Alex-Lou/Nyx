use gtk::prelude::*;
use gtk::{
    Box as GtkBox, Button, Orientation, Revealer, RevealerTransitionType, SearchEntry,
};
...
impl FindBar {
    pub fn new(tabs: &TabBar) -> Self {
        ...
        let findbar = Self { widget, entry, tabs: tabs.clone() };
    ...
    pub fn close(&self) {
        ...
        if let Some(wv) = self.tabs.current_webview() {
            wv.grab_focus();
        }
    }

    fn controller(&self) -> Option<FindController> {
        self.tabs.current_webview()?.find_controller()
    }
}
