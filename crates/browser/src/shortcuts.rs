...
/// Câble tous les raccourcis clavier de la fenêtre principale.
///
/// Ctrl+T  → nouvel onglet          Ctrl+W   → fermer onglet
/// Ctrl+L  → focus URL bar          Ctrl+R   → recharger
/// Ctrl+Tab → onglet suivant
pub fn wire(window: &ApplicationWindow, tabs: &TabBar, url_bar: &Entry) {
    ...
    bind(&accel, key::t.into_glib(), ctrl, flags, {
        let t = tabs.clone();
        move |_| { t.open_new_tab(); true }
    });

    bind(&accel, key::w.into_glib(), ctrl, flags, {
        let t = tabs.clone();
        move |_| { t.close_current(); true }
    });

    bind(&accel, key::r.into_glib(), ctrl, flags, {
        let t = tabs.clone();
        move |_| { t.with_current(|wv| wv.reload()); true }
    });

    bind(&accel, key::Tab.into_glib(), ctrl, flags, {
        let t = tabs.clone();
        move |_| { t.next_tab(); true }
    });
}

/// Raccourci vers `accel.connect_accel_group` avec une closure unaire.
